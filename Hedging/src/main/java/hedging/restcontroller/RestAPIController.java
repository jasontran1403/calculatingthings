...
public class RestAPIController {
	...
	public void hedging(List<Accounts> accounts) {
		List<Accounts> tempHedging = new ArrayList<>();

		for (int i = 0; i < accounts.size(); i++) {
			if (accounts.get(i).getBalance() >= accounts.get(i).getStoploss()
					&& accounts.get(i).getBalance() <= accounts.get(i).getStopprofit()) {
				tempHedging.add(accounts.get(i));
			}
		}

		if (tempHedging.size() == 1) {
			return;
		} else {
			if (tempHedging.size() % 2 != 0) {
				for (int i = 0; i < tempHedging.size()-1; i++) {
					listHedging.add(tempHedging.get(i));
				}
			} else {
				for (int i = 0; i < tempHedging.size(); i++) {
					listHedging.add(tempHedging.get(i));
				}
			}
		}
		
		System.out.println("Số tài khoản Hedging " + listHedging.size());
	...
	public void deleteAllData() {
		...
		transDAO.deleteAll();
		accDAO.deleteAll();
		System.out.println("Đã clear hết data");
	}
	...
	public List<Accounts> initiateData(@PathVariable("capital") double capital,
			@PathVariable("accountNumber") int accountNumber, @PathVariable("stoploss") double stoploss,
			@PathVariable("stopprofit") double stopprofit, @PathVariable("hedgingpercent") double hedgingpercent,
